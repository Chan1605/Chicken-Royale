...
- Hit 애니메이션 추가 (완)
- 플레이어 및 적 애니메이션 재구성 (불필요한 SetBool 대신 Trigger로 전환)(완)
- 버프,수류탄 제한두기 + UI 연동하기 (완)
- 지형 막아두기 및 이동반경 체크 터레인 크기 수정 및 파티클 오브젝트 위치 변경 (완 디테일 수정)
- 게임오버 판넬 생성 하기 -> 재도전 하시겠습니까? 다이얼로그 박스 나온뒤 yes or no 설정하기 (기능은 완 디테일작업 필요 -> 닷트윈 사용해서 UI애니메이션 적용)
- ESC 시 pause 생성 하기 ->  패널(UI)생성 (완료)
- 새로운 적 랜덤 생성 하기 (새로운 적 프리펩만 만들어서 연결)
- 조작법 가이드 생성 하기 (완료)
- 터레인 스카이박스 게임 시작 시 마다 랜덤하게 변경 (아침 저녁 새벽(완))
- 적 사망 시 힐 오브젝트 생성 및 이펙트 설정 (완료)
- 플레이어 최고점수 저장 PlayerPrefs(완료)
- Github Lfs 이용해서 작업했던 전체 프로젝트 커밋 (완료)
- 적이 사망 후 리젠 시 마테리얼 + 쉐이더 변경되도록 수정 (완료)
- 버프 활성화 시 총알이 지속시간동안 변경되도록 수정 (완료)
- 퍼즈 패널 게임 오버 패널 (완료)
- 최고점수 제거,타이틀로 돌아가기,게임재시작 시 팝업패널 생성(완료)
# 확인된 이슈 및 개선점 
