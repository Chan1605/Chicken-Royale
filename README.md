...
# DB 등록 후 QA 5/28
- [x] 리젠 되는 적의 스텟 버프 (사거리,이동속도,추적거리)
- [x] 플레이어의 스테미너 회복속도 버프
- [] (Mac) 빌드 후 실행 시 한글3자 입력후 엔터눌러야 닉네임설정됨 이슈 (영어는 상관없음) 
- [] 노트북 실행 시 로딩 및 타이틀화면에서 UI 해상도가 안맞는 이슈 (메인게임에서는 이슈없음)
- [] 간혈적으로 esc누를 때 일시정지 되면서 메뉴가 뜰 줄 알았는데 안뜬다. -> 확인 필요
